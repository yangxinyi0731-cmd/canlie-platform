...
# ============ 三、测试账号 ============
story.append(Paragraph("三、测试账号（统一密码 123456，已逐个验证可登录）", styles["h2"]))
story.append(table([
    ["角色", "手机号", "姓名/说明"],
...
story.append(table([
    ["#", "类别", "措施"],
    ["1", "网络安全", "修复验证码明文返回 + 硬编码 123456 漏洞（原可被用于接管任意账号）"],
...
story.append(Paragraph(
    "1. 接入真实短信服务（阿里云/腾讯云短信），当前验证码为模拟发送并打印在后端日志；上线后应轮换所有测试密码（现统一为 123456）。",
    styles["bodyIndent"]))
story.append(Paragraph(
